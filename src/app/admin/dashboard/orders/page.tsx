...
export default function OrdersPage() {
  ...
  return (
    <div className="space-y-6">
      <h1 className="text-2xl font-bold text-gray-800">{t('الطلبات الإلكترونية', 'Online Orders')}</h1>
      ...
      <div className="flex flex-wrap gap-2">
        {statuses.map((s) => (
          <button key={s} onClick={() => setFilter(s)}
            ...
            {t(statusLabels[s].ar, statusLabels[s].en)}
          </button>
        ))}
      </div>

      {filtered.length === 0 ? (
        <div className="bg-white rounded-xl shadow-sm border p-12 text-center text-gray-400"><p>{t('لا توجد طلبات', 'No orders')}</p></div>
      ) : (
          ...
          <div className="overflow-x-auto">
            <table className="w-full">
              <thead className="bg-gray-50 border-b">
                <tr>
                  <th className="text-right px-4 py-3 text-sm font-medium text-gray-600">{t('رقم الطلب', 'Order #')}</th>
                  ...
                  <th className="text-right px-4 py-3 text-sm font-medium text-gray-600">{t('الحالة', 'Status')}</th>
                  ...
                  <tr key={order._id} className="hover:bg-gray-50">
                    ...
                    <td className="px-4 py-3">
                      <select value={order.status} onChange={(e) => updateStatus(order._id, e.target.value)}
                        className={`text-xs px-2 py-1 rounded-full border-0 ${getStatusColor(order.status)}`} title={t('تحديث الحالة', 'Update status')}>
                        {statuses.filter((s) => s !== 'all').map((s) => <option key={s} value={s}>{t(statusLabels[s].ar, statusLabels[s].en)}</option>)}
                      </select>
                    </td>
                    <td className="px-4 py-3 text-sm text-gray-500">{formatDateShort(order.createdAt)}</td>
                    <td className="px-4 py-3">
                      <button onClick={() => setSelected(order)} className="text-sm text-[#5B7B6D] hover:underline">{t('التفاصيل', 'Details')}</button>
                    </td>
                  </tr>
                ))}
              </tbody>
        ...
        <>
          ...
          <div className="fixed inset-y-0 left-0 w-full max-w-lg bg-white z-50 shadow-2xl overflow-y-auto p-6">
            <div className="flex items-center justify-between mb-6">
              <h2 className="text-xl font-bold">{t('طلب', 'Order')} {selected.orderNumber}</h2>
              <button onClick={() => setSelected(null)} className="text-gray-500 hover:text-gray-700 text-xl">✕</button>
            </div>
            <div className="space-y-4">
              <div className="bg-gray-50 rounded-xl p-4">
                <h3 className="font-bold text-sm mb-2">{t('معلومات العميل', 'Customer info')}</h3>
                ...
                {selected.customer.notes && <p className="text-sm text-gray-500 mt-1">{t('ملاحظات', 'Notes')}: {selected.customer.notes}</p>}
              </div>
              <div>
                <h3 className="font-bold text-sm mb-2">{t('المنتجات', 'Items')}</h3>
              ...
              <div>
                <label className="block text-sm font-medium mb-1">{t('تحديث الحالة', 'Update status')}</label>
                <select value={selected.status} onChange={(e) => updateStatus(selected._id, e.target.value)}
                  className="w-full px-4 py-2 border border-gray-300 rounded-xl text-sm" title={t('تحديث الحالة', 'Update status')}>
                  {statuses.filter((s) => s !== 'all').map((s) => <option key={s} value={s}>{t(statusLabels[s].ar, statusLabels[s].en)}</option>)}
                </select>
              </div>
              <p className="text-xs text-gray-400">{t('تاريخ الطلب', 'Order date')}: {formatDateShort(selected.createdAt)}</p>
