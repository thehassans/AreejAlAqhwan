...
export default function WorkerSettingsPage() {
  ...
  return (
    <div className="space-y-5 max-w-lg">
      <div>
        <h1 className="text-xl font-bold text-gray-800">{t('معلومات المتجر', 'Store info')}</h1>
        <p className="text-sm text-gray-400 mt-0.5">{t('بيانات المتجر الأساسية', 'Basic store details')}</p>
      </div>
      ...
      <div className="bg-[#5B7B6D]/5 border border-[#5B7B6D]/10 rounded-2xl p-4 flex items-start gap-3">
        ...
        <p className="text-sm text-[#5B7B6D]">{t('هذه الصفحة للعرض فقط. لتعديل الإعدادات يرجى التواصل مع المسؤول.', 'This page is read-only. Contact the admin to change settings.')}</p>
      </div>

      {settings && (
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm divide-y divide-gray-50">
          <div className="p-4 flex items-center justify-between">
            <span className="text-sm text-gray-500 flex items-center gap-2"><FiSettings size={14} /> {t('اسم المتجر', 'Store name')}</span>
            <span className="text-sm font-semibold text-gray-800">{settings.storeName || '—'}</span>
          </div>
          <div className="p-4 flex items-center justify-between">
            <span className="text-sm text-gray-500">{t('رقم الهاتف', 'Phone')}</span>
            <span className="text-sm font-semibold text-gray-800" dir="ltr">{settings.phone || '—'}</span>
          </div>
          <div className="p-4 flex items-center justify-between">
            <span className="text-sm text-gray-500">{t('العنوان', 'Address')}</span>
            <span className="text-sm font-semibold text-gray-800">{settings.address || '—'}</span>
          </div>
          <div className="p-4 flex items-center justify-between">
            <span className="text-sm text-gray-500">{t('ضريبة القيمة المضافة', 'Value Added Tax')}</span>
            <span className={`text-xs px-2 py-1 rounded-lg font-semibold ${settings.vatEnabled ? 'bg-emerald-50 text-emerald-700' : 'bg-gray-50 text-gray-500'}`}>
              {settings.vatEnabled ? t(`${settings.vatPercentage}% مفعّلة`, `${settings.vatPercentage}% enabled`) : t('غير مفعّلة', 'Disabled')}
            </span>
          </div>
        </div>
      )}
